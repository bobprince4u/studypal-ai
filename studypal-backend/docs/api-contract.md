...
| **7.3** | `POST /api/session`, `POST /api/ask` | any length username accepted | `>200` chars → `400 {"error":"Username must be 200 characters or fewer"}` | Unbounded identifier written to the database on an unauthenticated endpoint. 200 chars (configurable via `MAX_USERNAME_LENGTH`) is far above any real name or student id. The message is specific rather than reusing `"Username required"` because the frontend renders `data.error` verbatim on the login screen. |
